...
import { CheckCircle2, Droplets, ShieldCheck, Zap } from 'lucide-react';
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] shadow-2xl">
            ...
            <div className="absolute top-6 left-6 flex gap-2">
              <span className="rounded-full bg-(--accent) px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest text-white shadow-lg">
                {campaign.category}
              </span>
            ...
            {(campaign.creatorName || campaign.creatorEmail) && (
              <div className="flex items-center gap-4 border-y border-(--border) py-6">
                ...
                <div className="flex-1">
                  <p className="font-bold text-(--foreground)">{campaign.creatorName ?? 'Campaign Creator'}</p>
                  <p className="text-xs font-medium text-(--muted)">{campaign.creatorEmail}</p>
                </div>
          ...
          <article className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-6 pt-6 sm:grid-cols-2">
              <div className="space-y-4 rounded-3xl border border-(--border) bg-(--surface) p-8 shadow-sm">
                ...
                <h4 className="text-xl font-bold text-(--foreground)">Reward Info</h4>
              ...
              <div className="space-y-4 rounded-3xl border border-(--border) bg-(--surface) p-8 shadow-sm">
                ...
                <h4 className="text-xl font-bold text-(--foreground)">Minimum Contribution</h4>
                <p className="text-sm leading-relaxed text-(--muted)">
                  Support this campaign with at least {(campaign.minimumContribution ?? 0).toLocaleString()} credits.
                </p>
              </div>
            </div>
          </article>
