...
export default function DonationWidget({ campaign }: DonationWidgetProps) {
  ...
  const handleContinue = () => {
    ...
    // with { campaignId: campaign.id, amount } and redirect the browser to
    // the returned session URL.
    console.log('Continue to payment', { campaignId: campaign._id, amount });
  };
  ...
  return (
    ...
    >
      <p className="text-sm font-semibold uppercase tracking-[0.28em] text-(--accent)">Support this campaign</p>
      <h3 className="mt-2 text-lg font-semibold text-(--foreground)">Choose an amount</h3>
      ...
      <div className="mt-4">
        <label htmlFor="custom-amount" className="text-xs font-medium text-(--muted)">
      ...
      <p className="mt-3 text-center text-xs text-(--muted)">Payments are securely processed by Stripe.</p>
    </motion.div>
  );
}